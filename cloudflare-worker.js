...
async function callAIToTailor(env, masterCv, jdText, hashtag) {
  const systemPrompt = [
    ...
    "   - 'projects': Review all candidate's projects from master CV data (Job Portal, Student Management, E-Commerce Platform, Automated CV Builder/AI). For each project, keep 'role' strictly as 'Developer' (MANDATORY: DO NOT change 'role' to any other value; it MUST always be 'Developer'). Rewrite 'desc', and especially the 'tasks' array. Align these tasks with the requirements in the JD by emphasizing matching tech stacks, methodologies (e.g. Agile/Scrum, Event-driven, Microservices, Testing, AI workflow), and business/performance outcomes. Ensure you include target keywords from the JD, but do not invent fake facts or projects. Select the 3 most relevant projects from master CV to keep in the final data.",
